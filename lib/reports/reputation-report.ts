...
export type ReputationReportRecord = {
  company_id: string
  company_name: string
...
export async function getOrCreateReputationReport(
  companyId: string,
  accountId: string,
  ...
  let cachedReport: Partial<ReputationReportRecord> | null = null
  ...
  if (!refresh) {
    ...
    if (data) {
      ...
      return data as ReputationReportRecord
    }
  } else {
    const { data } = await db
      .from('reputation_reports')
    ...
    cachedReport = data as Partial<ReputationReportRecord> | null
  }
  ...
  const reviewVelocity = company.review_count ? parseFloat((company.review_count / 24).toFixed(1)) : 0
  const avgLocalReviews = 85
  const visibilityScore = Math.min(100, Math.round(((company.review_count ?? 0) / avgLocalReviews) * 100))
  ...
  const report: ReputationReportRecord = {
    ...
    share_token: cachedReport?.share_token ?? crypto.randomUUID(),
  ...
  await db
    ...
    .then(({ error }) => {
      if (error) console.warn('[report] cache upsert skipped:', error.message)
    })
