 ...
 *
 * Real: const run = await client.actor(actorId).call(input)
 */
...
/**
 * Poll and fetch the dataset from a completed actor run.
 *
 * Real: const { items } = await client.dataset(run.defaultDatasetId).listItems()
 */
