...
-- This migration is safe to run against partially provisioned databases.
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'account_memberships') THEN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'account_invitations') THEN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'account_settings') THEN
    ...
    DROP POLICY IF EXISTS account_settings_insert_policy ON public.account_settings;
    CREATE POLICY account_settings_insert_policy
    ON public.account_settings
    FOR INSERT
    WITH CHECK (public.has_account_role(account_id, ARRAY['owner','admin']));
